...
kern_return_t offsets_determine_initializer_for_device_and_build(char * device, char * build, init_func * func) {
	...
	if (strstr(device, "iPhone8,2"))
	{
		DEBUG_LOG("Detected iPhone 6s");
		if (strstr(build, "14D27"))
		{
			DEBUG_LOG("Initializing for iOS 10.2.1");
			*func = (init_func)init_iphone_6_14d27;
			ret = KERN_SUCCESS;
		}
		else {
